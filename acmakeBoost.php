...
if ($file_name == "main.c") {
  ...
  $cpp_program = <<<'PROGRAM'
    ...
    void greet(const ProgrammingLanguage<T> &pl) {
      ...
      std::string author{pl.author};
      std::string message{boost::str(boost::format("Happy programming with %1%, created by %2%!") %name % author)};
      std::cout << message << '\n';
    }

    int main() {
      ProgrammingLanguage<std::string_view> cpp{"C++", "Bjarne Stroustrup"};
      greet(cpp);
      return 0;
    }    
    PROGRAM;
...
$boost_installed = shell_exec("dpkg -s libboost-all-dev | grep Status");

if (strpos($boost_installed, "install ok installed") == false){
    echo "Boost C++ library is not installed on your system.\n";
    echo "Downloading and installing Boost C++ library...\n";
    shell_exec("sudo apt-get update -y");
    shell_exec("sudo apt-get install libboost-all-dev -y");
} 
...
if ($extension === 'c') {
  ...
  # Link Boost library to executable
